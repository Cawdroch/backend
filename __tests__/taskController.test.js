...
test("should return a 401 error if user is not found", async () => {
  ...
  const userId = "non-exiatnat-user-id";
  const req = {
    params: { id: taskId },
    user: { id: userId },
  ...
  // Mockin the findById method to return null, indicating user not found
...
test("should return a 401 error if the user is not authorized to update the task", async () => {
  const taskId = "task-id-1";
  ...
  // Mocking task to be updated, owned by a different user
